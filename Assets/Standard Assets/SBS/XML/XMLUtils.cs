...
namespace SBS.XML
{
	public class XMLUtils
	{
        public static int ParseInt(XMLNode element)
        {
            if (element == null)
            {
                //.Assert(false);//, "Invalid node name: " + element.tagName + " in: " + element.parentNode.tagName);
                return 0;
            }
        ...
        public static float ParseFloat(XMLNode element)
        {
            if (element == null)
            {
                //.Assert(false);//, "Invalid node name: " + element.Name + " in: " + element.ParentNode.Name);
                return float.NaN;
            }
...
//            if (parts.Length < 3)
                //.Assert(false);//, "Invalid Vector3 param length node: " + element.Name + " in: " + element.ParentNode.Name);
