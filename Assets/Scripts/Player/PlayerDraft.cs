...
public class PlayerDraft : MonoBehaviour
{
    ...
    void ChangeColorBySlipstreamLevel(int colorIndex)
    {
        ...
        for (int i = 0; i < currentFxs.Count; ++i)
        {
            if (currentFxs[i].gameObject.transform.Find("fx_1") != null)
            {
                currentFxs[i].gameObject.transform.Find("fx_1").GetComponent<ParticleSystem>().startColor = currentColor;
                currentFxs[i].gameObject.transform.Find("fx_2").GetComponent<ParticleSystem>().startColor = currentColor;
                if (currentFxs[i] == slipstreamFX_type_3)
                {
                    currentFxs[i].gameObject.transform.Find("fx_1/Fx_Blue_Strobo").GetComponent<ParticleSystem>().startColor = currentColor;
                    currentFxs[i].gameObject.transform.Find("fx_2/Fx_Blue_Strobo").GetComponent<ParticleSystem>().startColor = currentColor;
                    currentFxs[i].gameObject.transform.Find("fx_1/Fx_Iceball").GetComponent<ParticleSystem>().startColor = currentColor;
                    currentFxs[i].gameObject.transform.Find("fx_2/Fx_Iceball").GetComponent<ParticleSystem>().startColor = currentColor;
                }
            }
            else
            {
    ...
    void ChangeHorizontalOffset(ParticleSystem effect)
    {
        if (effect.name.StartsWith("Fx_Slipstream1"))
        {
            ...
            Transform currFx = effect.gameObject.transform.Find("fx_1").transform;
            currFx.localPosition = new Vector3(horOffset[0], currFx.localPosition.y, currFx.localPosition.z);
            currFx = effect.gameObject.transform.Find("fx_2").transform;
            currFx.localPosition = new Vector3(-horOffset[0], currFx.localPosition.y, currFx.localPosition.z);
        }
    }
