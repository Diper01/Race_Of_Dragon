...
public class OnTheRunUITransitionManager : Manager<OnTheRunUITransitionManager>
{
    ...
    void Awake()
    {
        ...
        StartPageNavBar[0] = transform.Find("StartPage/AnchorBottomLeft").gameObject;
        StartPageNavBar[1] = transform.Find("StartPage/AnchorTop").gameObject;
        StartPageNavBar[2] = transform.Find("StartPage/BgDown").gameObject;
        StartPageNavBar[3] = transform.Find("StartPage/Logo/otr_logo_main").gameObject;
        StartPageNavBar[4] = transform.Find("StartPage/AnchorBottomRight").gameObject;
        StartPageNavBar[5] = transform.Find("StartPage/CenterAnchor").gameObject;
        StartPageNavBar[6] = transform.Find("StartPage/Logo/bg_black_alpha").gameObject;

        TopNavBar[0] = transform.Find("CurrencyBar/CoinsItem").gameObject;
        TopNavBar[1] = transform.Find("CurrencyBar/DiamondsItem").gameObject;
        TopNavBar[2] = transform.Find("CurrencyBar/FuelItem").gameObject;
        TopNavBar[3] = transform.Find("OffgameBG/BGup").gameObject;
        TopNavBar[4] = transform.Find("TopLeftObjects/btHome").gameObject;
        TopNavBar[5] = transform.Find("TopLeftObjects/LevelBar").gameObject;
        TopNavBar[6] = transform.Find("CurrencyBar/linksBar").gameObject;
...
#else
        BottomNavBar[0] = transform.Find("ButtonsBar/btRankings").gameObject;
        BottomNavBar[1] = transform.Find("ButtonsBar/SpinWheelButton").gameObject;
#endif
        BottomNavBar[3] = transform.Find("ButtonsBar/btTrucks").gameObject;
        BottomNavBar[4] = transform.Find("GaragePage/AnchorBottomRight").gameObject;
        BottomNavBar[5] = transform.Find("OffgameBG/BgDown").gameObject;
        BottomNavBar[6] = transform.Find("ButtonsBar/btTiers").gameObject;

        transform.Find("RewardPage").GetComponent<UIRewardPage>().bottomBar = transform.Find("RewarBottomBar").gameObject;
    }
...
//                    OnTheRunInterstitialsManager.Instance.TriggerInterstitial(OnTheRunInterstitialsManager.TriggerPoint.BackToMainScreen);
            ...
            case "RewardPage":
                ...
                else
                {
                    ...
                    uiRoot.ShowBottomPageBorders(false);
                    UIManager.Instance.ActivePage.GetComponent<UIRewardPage>().transform.Find("CenterScreenAnchor/AnimAnchor").SendMessage("StartEnterExitAnimation", UIEnterExitAnimations.AnimationType.EnterPage);
                    UIManager.Instance.ActivePage.GetComponent<UIRewardPage>().transform.Find("CenterScreenAnchor/MissionRewardPanel").SendMessage("StartEnterExitAnimation", UIEnterExitAnimations.AnimationType.EnterPage);
    ...
    IEnumerator ClosePopupDelayed()
    {
        ...
        if (openAnotherPopup != "")
        {
            ...
            {
                OpenPopup(openAnotherPopup);
                uiManager.FrontPopup.transform.Find("content/popupTextField").GetComponent<UITextField>().text = anotherPopupText;
    ...
    public void OpenBuyPopup(string titleText, string descriptionText)
    {
        ...
        uiManager.FrontPopup.transform.Find("content/titletText").GetComponent<UITextField>().text = titleText;
        uiManager.FrontPopup.transform.Find("content/descriptionTextField").GetComponent<UITextField>().text = descriptionText;
        uiManager.FrontPopup.transform.Find("content/ResumeButton").GetComponentInChildren<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("ok");
        uiManager.FrontPopup.transform.Find("content/ResumeButton").GetComponent<UIButton>().onReleaseEvent.AddTarget(gameObject, "OnBuyPopupClosed");
    }
    ...
    public void OnMoveTopNavBar(UIEnterExitAnimations.AnimationType type, float delay = 0.0f)
    {
        foreach (GameObject go in TopNavBar)
        {
            if (go.activeInHierarchy)
            {
                // fixed bug Remove Energy (add if(){})
                if (go.GetComponent<UIEnterExitAnimations>())
                {
                    if (delay != 0.0f)
