...
public class UIWheelPopup : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        wheelRb = transform.Find("content/wheel/inner").gameObject.GetComponent<Rigidbody2D>();
        wheelHinge = transform.Find("content/wheel/inner").gameObject.GetComponent<HingeJoint2D>();
        ...
        OnTheRunUITransitionManager.Instance.InitializeBackground(transform.Find("background/bg_black_gradient"));
        ...
        extraSpinButton.transform.Find("tfTextfield").GetComponent<UITextField>().text = extraSpinCost.ToString();
        transform.Find("content/texts/tfTitle").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("spin_and_win");
        transform.Find("content/texts/tfStill").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("still");
        transform.Find("content/texts/tfExtraSpin").GetComponent<UITextField>().text = extraSpinGained + " " + OnTheRunDataLoader.Instance.GetLocaleString("extra_spins");
        transform.Find("content/texts/SpinButton").GetComponentInChildren<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("spin");
    ...
    void InitializeWheel()
    {
        ...
        if (lastWheelSpinConfigDate.Length > 0)
        {
            ...
            for (int i = 0; i < 8; ++i)
            {
               ...
               //Debug.Log("--- " + wheelItem.item + " | " + wheelItem.level + " | " + wheelItem.quantity);
           }
    ...
    void RefreshWheelGraphic()
    {
        ...
        {
            Transform currentSliceIcons = slices[i].Find("icons");
            for (int j = 0; j < currentSliceIcons.childCount; ++j)
            {
                ...
                if (currItem == wheelItems[i].item)
                {
                    ...
                    if (currItem == UIWheelItem.WheelItem.FuelFreeze)
                        currentSliceIcons.parent.Find("text").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("freeze");
                    else
                        currentSliceIcons.parent.Find("text").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(wheelItems[i].quantity);
    ...
    void UpdateInterface()
    {
        ...
        transform.Find("content/texts/SpinButton").GetComponentInChildren<UITextField>().ApplyParameters();
    }
    ...
    void OnEndSpinEnter()
    {
        ...
        Transform currentSliceIcons = currentSlice.Find("icons");
        for (int j = 0; j < currentSliceIcons.childCount; ++j)
        {
...
*/
    ...
    void OnCollectRewardEnter()
    {
        ...
        UIWheelFeedbackPopup currPopupComp = uiManager.FrontPopup.gameObject.GetComponent<UIWheelFeedbackPopup>();
        currPopupComp.numText.text = Manager<UIRoot>.Get().FormatTextNumber(wheelItems[lastPrizeIndex].quantity);
        uiManager.FrontPopup.transform.Find("content/CollectButton").GetComponent<UIButton>().onReleaseEvent.AddTarget(gameObject, "OnPopupClosed");
        ...
        OnTheRunFireworks.Instance.StartFireworksEffect(25, transform.Find("content/fireworks"));
...
*/
    ...
    void UpdateDragArea()
    {
        ...
        BoxCollider dragAreaBtn = transform.Find("content/wheel/dragAreaBtn").GetComponent<BoxCollider>();
        Vector3 wheelCenterPos = transform.Find("content/wheel").position; // dragAreaBtn.bounds.center;
        ...
        Vector3 bgLabelPos = uiManager.UICamera.WorldToScreenPoint(transform.Find("content/wheel/bg_label").position);
    ...
    void ShowNoMoreSpinPopup(bool forceShow=false)
    {
        if (forceShow || backFromCurrencyPopup) //!noSpinPopupShowed || 
        {
            ...
            uiManager.FrontPopup.transform.Find("content/popupTitle").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("no_spin_popup_title");
            uiManager.FrontPopup.transform.Find("content/popupTextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("no_spin_popup_text");
            uiManager.FrontPopup.transform.Find("content/tfExtraSpin").GetComponent<UITextField>().text = "BUY NOW 3 EXTRA SPINS!";
            uiManager.FrontPopup.transform.Find("content/SkipButton/TextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("skip");
            uiManager.FrontPopup.transform.Find("content/ExtraSpinButton/tfTextfield").GetComponent<UITextField>().text = extraSpinCost.ToString();
        }
    }
