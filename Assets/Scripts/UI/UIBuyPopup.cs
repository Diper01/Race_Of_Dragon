...
public class UIBuyPopup : MonoBehaviour
{
    ...
    void Awake()
    {
        nameText = transform.Find("nameTextField").GetComponent<UITextField>();
        shortDescrText = transform.Find("descrTextField").GetComponent<UITextField>();
        buyButtonText = transform.Find("BuyPanel/BuyButton/PriceTextField").GetComponent<UITextField>();
        upgradeButtonText = transform.Find("UpgradePanel/InfoItem/UpgradeButton/tfTextfield").GetComponent<UITextField>();
    }
    ...
    */
    ...
    void UpdatePopupStatus()
    {
        ...
        transform.Find("BuyPanel/BuyButton/Currency/coin").gameObject.SetActive(buyCarCurrency == PriceData.CurrencyType.FirstCurrency);
        transform.Find("BuyPanel/BuyButton/Currency/diamond").gameObject.SetActive(buyCarCurrency == PriceData.CurrencyType.SecondCurrency);
        transform.Find("nameTextField").GetComponent<UITextField>().text = OnTheRunGameplay.GetCarLocalizedStr(currSpecialCarData.type);
        ...
        transform.Find("descrTextField").GetComponent<UITextField>().text = OnTheRunGameplay.GetCarLocalizedDescrStr(truckType);
        string currencyStr = dataLoader.GetLocaleString("diamonds");
#if UNITY_WEBPLAYER
        currencyStr = OnTheRunDataLoader.Instance.GetLocaleString("coins");
#endif       
        transform.Find("BuyPanel/buyTextField").GetComponent<UITextField>().text = dataLoader.GetLocaleString("buy_for");
        transform.Find("UpgradePanel/tfTextLabel").GetComponent<UITextField>().text = dataLoader.GetLocaleString("duration");
        ...
        {
            transform.Find("BuyPanel").gameObject.SetActive(false);
            transform.Find("UpgradePanel").gameObject.SetActive(false);
            buyButtonText.text = Manager<UIRoot>.Get().FormatTextNumber(currSpecialCarData.cost);
            transform.Find("descrTextField2").gameObject.SetActive(true);
            ...
            transform.Find("descrTextField2").GetComponent<UITextField>().text = descrStr;
        }
        else
        {
            transform.Find("BuyPanel").gameObject.SetActive(false);
            transform.Find("UpgradePanel").gameObject.SetActive(true);
            ...
            transform.Find("descrTextField2").gameObject.SetActive(false);
        }
    }

    void FillDurationBar(int value, int minValue, int maxValue, bool forceEmpty = false)
    {
        Transform parent = transform.Find("UpgradePanel/InfoItem");
        GameObject infoBar = parent.Find("Bar").gameObject;

        Transform upgradeButton = parent.Find("UpgradeButton");
        upgradeButton.GetComponent<UIButton>().State = UIButton.StateType.Normal;
        upgradeButton.Find("coin").gameObject.SetActive(upgradeCarCurrency == PriceData.CurrencyType.FirstCurrency);
        upgradeButton.Find("diamond").gameObject.SetActive(upgradeCarCurrency == PriceData.CurrencyType.SecondCurrency);
        ...
        for (int i = minValue; i < maxValue; i++)
        {
            ...
            currElement.Find("empty").gameObject.SetActive(isEmpty || forceEmpty);
    ...
    void Signal_OnInfoReleased(UIButton button)
    {
        ...
        uiManager.FrontPopup.gameObject.transform.Find("content/popupTextField").GetComponent<UITextField>().text = OnTheRunGameplay.GetCarLocalizedLongDescrStr(currSpecialCarData.type);
    }
}