...
public class UICurrencyPopup : MonoBehaviour
{
    ...
    //-------------------------Duc---------------------------
    public GameObject[] tpriceDiamon;
    public GameObject[] tpriceMonney;
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < 6; i++)
            items[i] = transform.Find("content/ShopItem" + (i + 1).ToString()).gameObject.GetComponent<UICurrencyShopItem>();
    ...
    void Signal_OnAddCurrencyRelease(UIButton button)
    {
        ...
        switch (currentPopupType)
        {
            case ShopPopupTypes.Diamonds:
                ...
                //				Debug.Log("-----------------Buy Diamon in Shop ------------------ " + tempDiamondsIndex);

                switch (tempDiamondsIndex)
                {
                    case 0:
                        //	BuyIAP_8999();
                        Debug.Log("-----------------Buy Diamon in Shop ------------------ " + tempDiamondsIndex);
                ...
                //				PlayerPersistentData.Instance.Diamonds += 10;
    ...
    void Initialize(UICurrencyPopup.ShopPopupTypes popupType)
    {
        ...
        for (int i = 0; i < items.Length; i++)
        {
            ...
            if (currentQuantity[i] > 0)
            {
                ...
                {
                    tpriceDiamon[i].SetActive(true);
                    items[i].EnableButton();
                    tpriceMonney[i].SetActive(false);
                    print("---------Diamon-----------");
                ...
                {
                    tpriceMonney[i].SetActive(true);
                    tpriceDiamon[i].SetActive(false);
                    print("---------Money-----------");
                    ...
                    if (i == freeCoinsIndex)
                    {
                        ...
                        //						if(AdColony.IsVideoAvailable("vz7d765f748a374cb48e"))
                        //						{
                        //							Debug.Log("Playing AdColony Video");
                        //							videoAvailable = true;
                        //							freeVideoAds = false;
                        //						}
                        //						else if (Advertisement.IsReady())
                        //						{
                        //							Debug.Log("Adcolony is not Not Available");
                        //							videoAvailable = true;
                        //							freeVideoAds = true;
                        //						}
                        //						else
    ...
    void ShowFeedbackPopup(string text)
    {
        ...
        popup.transform.Find("content/popupTextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("you_bought") + " " + text;
        popup.transform.Find("content/OkButton").gameObject.SetActive(true);
    }
    ...
    public const string IAP_099 = "buy.iap.0.99";
    public const string IAP_199 = "buy.iap.1.99";
    public const string IAP_599 = "buy.iap.5.99";
    public const string IAP_1099 = "buy.iap.10.99";
    public const string IAP_4999 = "buy.iap.49.99";
    public const string IAP_8999 = "buy.iap.89.99";
