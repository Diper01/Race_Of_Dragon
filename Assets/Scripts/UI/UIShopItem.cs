...
public class UIShopItem : MonoBehaviour
{
	...
	void Awake()
	{
		BuyButton = transform.Find("BuyBoosterButton").gameObject;
        ...
        FxSoldOut.transform.Find("SoldOut").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("item_purchased");
		...
		CoinIcon = transform.Find("BuyBoosterButton/coin").gameObject;
        DiamondIcon = transform.Find("BuyBoosterButton/diamond").gameObject;
		ButtonPriceTf = transform.Find("BuyBoosterButton/tfTextfield").GetComponent<UITextField>();
	...
	public void GivePrize()
	{
		...
		if (bought && !boosterRef.equipped)
		{
			...
			transform.Find("BuyBoosterButton").gameObject.SetActive(false);
    ...
    {
        //Manager<UIManager>.Get().GetPage("InAppPage").SendMessage("UpdateAllButtons");
        this.transform.Find("BuyBoosterButton").gameObject.SetActive(true);
    ...
    void Signal_OnInfoReleased(UIButton button)
    {
        ...
        Manager<UIManager>.Get().FrontPopup.gameObject.transform.Find("content/popupTitle").GetComponent<UITextField>().text = button.transform.parent.GetComponent<UIShopItem>().boosterRef.displayedName;
        Manager<UIManager>.Get().FrontPopup.gameObject.transform.Find("content/popupTextField").GetComponent<UITextField>().text = button.transform.parent.GetComponent<UIShopItem>().boosterRef.description;
    }
    ...
    void Signal_OnMoreOver(UIButton button)
    {
        ...
        shopItemTr.Find("BuyBoosterButton").GetComponent<UIButton>().State = UIButton.StateType.Pressed;
    ...
    void Signal_OnMoreOut(UIButton button)
    {
        ...
        if(button.name.Contains("ItemButton"))
            shopItemTr = button.transform.parent.parent;
        shopItemTr.Find("BuyBoosterButton").GetComponent<UIButton>().State = UIButton.StateType.Normal;
    }
...
*/
    ...
    void Signal_OnItemPressed(UIButton button)
    {
        ...
        if (shopItemTr.Find("BuyBoosterButton").gameObject.activeInHierarchy)
        {
            interfaceSounds.SendMessage("PlayGeneralInterfaceSound", OnTheRunInterfaceSounds.InterfaceSoundsType.Click);
...
*/
	...
	void OnBoosterPurchase(bool bought, Transform shopItemTr) //mark
	{
		...
		if (bought && !boosterRef.equipped)
		{
            ...
            boosterRef.TryEquipping();
			UpdateInAppButton(shopItemTr);
			shopItemTr.Find("BuyBoosterButton").gameObject.SetActive(false);
			Manager<UIManager>.Get().GetPage("InAppPage").SendMessage("UpdateAllButtons");
			FxStars.GetComponent<ParticleSystem>().Play();
			FxSoldOut.SetActive( false );
			FxSoldOut.SetActive( true );
    ...
    void EquipAndUpdateInAppButton(Transform shopItemTr)
    {
        ...
        {
            UpdateInAppButton(shopItemTr);
            shopItemTr.Find("BuyBoosterButton").gameObject.SetActive(!boosterRef.equipped);
        }
        else
        {
            if (boosterRef.used)
            {
                shopItemTr.Find("Background_normal").gameObject.SetActive(true);
                shopItemTr.Find("Background_active").gameObject.SetActive(false);
                shopItemTr.Find("BuyBoosterButton").gameObject.SetActive(false);
            }
        }

        if (boosterRef.type == OnTheRunBooster.BoosterType.SpecialCar && boosterRef.equipped)
            shopItemTr.Find("bg_selection_prize").gameObject.SetActive(false);
    }

    void UpdateInAppButton(Transform shopItemTr)
    {

        shopItemTr.Find("Background_normal").gameObject.SetActive(!boosterRef.equipped);
        shopItemTr.Find("Background_active").gameObject.SetActive(boosterRef.equipped);
    }
