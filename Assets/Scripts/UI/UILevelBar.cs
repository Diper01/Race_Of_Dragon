...
public class UILevelBar : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tfLevelValue = transform.Find("tfLevelValue").gameObject.GetComponent<UITextField>();
        tfCompleteLevel = transform.Find("tfCompleteLevel").gameObject.GetComponent<UITextField>();

        expBar = transform.Find("Bar/bar_green_bg").gameObject.GetComponent<UINineSlice>();
        expBar.width = 0.0f;

        totalBarWidth = transform.Find("Bar/bar_top_bg").gameObject.GetComponent<UINineSlice>().width;
        particleEffect = transform.Find("FxStars2D");
    ...
    void OnEnable()
    {
        ...
        if (PlayerPersistentData.Instance.Level == 35) tfLevelValue.text = "35";
    }
    ...
    public void UpdateExperienceBarImmediatly(int experienceGained)
    {
        ...
        transform.Find("Bar/bar_green_bg").gameObject.SetActive(PlayerPersistentData.Instance.Experience > 0.0f);
    ...
    public void UpdateExperienceBarAnimated(float delay, int experienceGained, float speed = 1.0f)
    {
        ...
        if (delay > 0.0f)
        {
            ...
            currentExperienceGained = 5000;// experienceGained;
        }
        else
        {
            UpdateExperienceBarImmediatly(0);
            transform.Find("Bar/bar_green_bg").gameObject.SetActive(true);
    ...
    {
        if (GameObject.Find("RewardPage")!=null&&GameObject.Find("RewardPage").activeInHierarchy) return;
        if (PlayerPersistentData.Instance.Level>=35) return;
        ...
        if ((uiManager.ActivePageName != "RewardPage" || !UIRewardPage.firstStep) && !wheelIsSpinning)
        {
            ...
            Manager<UIManager>.Get().FrontPopup.gameObject.transform.Find("content/popupTitle").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("rank_bar_title");
