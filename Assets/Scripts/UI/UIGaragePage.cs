...
public class UIGaragePage : MonoBehaviour
{
    ...
    void InitializePage()
    {
        ...
        changingBackground = Manager<UIRoot>.Get().transform.Find("OffgameBG/BgCentral/bg");
        changingBackgroundCenter = Manager<UIRoot>.Get().transform.Find("OffgameBG/BgCentral/bg/Center").GetComponent<SpriteRenderer>();
        changingBackgroundTop = Manager<UIRoot>.Get().transform.Find("OffgameBG/BgCentral/bg/Up").GetComponent<SpriteRenderer>();
        changingBackgroundBottom = Manager<UIRoot>.Get().transform.Find("OffgameBG/BgCentral/bg/Down").GetComponent<SpriteRenderer>();
        ...
        purchaseCar = transform.Find("AnchorCenter/purchaseCar").gameObject;
        purchaseCarLocked = transform.Find("AnchorCenter/LockedPanel/purchaseLockedCar").gameObject;
        purchaseCarUnlocked = transform.Find("AnchorCenter/purchaseUnlockedCar").gameObject;

        lockedPanel = transform.Find("AnchorCenter/LockedPanel").gameObject;
        ...
        transform.Find("AnchorBottomRight/PlayButton/TextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("btPlay");
        transform.Find("LeftCenterAnchor/InfoPanel/ParkingLotLocked/tfGarageTitle").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("buy_parking_lot");

        purchaseCarLocked.transform.Find("tfBuyLabel").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("or");
        purchaseCarUnlocked.transform.Find("tfOrLabel").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("or");
        ...
        tfGarageTitle = transform.Find("AnchorCenter/Locations/tfGarageTitle").GetComponent<UITextField>();
        uiButtonsBar = GameObject.Find("UI").transform.Find("ButtonsBar").gameObject;

        currentCarsList = Cars_EU;
        parkingAreaWidth = 20.0f;
        carSpeed = 50.0f;

        uiPlayButton = gameObject.transform.Find("AnchorBottomRight/PlayButton").GetComponent<UIButton>();
        playArrowEnable = uiPlayButton.transform.Find("Arrow_enable").gameObject;
        playArrowEnable.SetActive(true);
        playArrowDisable = uiPlayButton.transform.Find("Arrow_disable").gameObject;
        playArrowDisable.SetActive(false);

        carStatsGroup = transform.Find("LeftCenterAnchor/InfoPanel/StatsPanel");

        infoBars = new Transform[infoBarNum];
        for (int i = 0; i < infoBarNum; i++)
		{
            infoBars[i] = carStatsGroup.Find("CarInfoItem" + (i + 1));
		}

        carRentTimer = transform.Find("AnchorCenter/CarRentTimer").gameObject;
        comingSoonPanel = transform.Find("LeftCenterAnchor/InfoPanel/ComingSoon").gameObject;
        parkingLotLocker = transform.Find("LeftCenterAnchor/InfoPanel/ParkingLotLocked").gameObject;
        parkingLotLockerWeb = transform.Find("LeftCenterAnchor/InfoPanel/ParkingLotLockedWeb").gameObject;
        parkingLotLockerXP = transform.Find("LeftCenterAnchor/InfoPanel/ParkingLotLockedXP").gameObject;
        parkingLotLockedButton = parkingLotLocker.transform.Find("BuyParkingLotButton").gameObject;
        parkingLotBuyForButton = parkingLotLockerXP.transform.Find("BuyParkingLotButton").gameObject;
        ...
        transform.Find("InvisibleButtonRightWeb").gameObject.SetActive(false);
        transform.Find("InvisibleButtonLeftWeb").gameObject.SetActive(false);
    ...
    void UpdateVehiclesPositionScale()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            mirroredCar = scrollerCarsList[i].transform.parent.Find("Mirrored");
            if (mirroredCar != null)
    ...
    void ActivatePanel(string panelName, bool active)
    {
        switch (panelName)
        {
			...
			if( active )
			{
				...
				purchaseCarLocked.transform.Find("PurchaseButton").gameObject.SetActive( true );
				
				Transform p = purchaseCarLocked.transform.Find("PurchaseButton").gameObject.transform.parent.Find("tfBuyLabel");
				if( p != null )
            ...
            case "InfoPanel":
                ...
                {
                    UIButton currButton = infoBars[i].Find("GreenButtonSquare").transform.GetComponent<UIButton>();
    ...
    void ActivatePlayer(GameObject playerRef, bool activate)
    {
        ...
        GameObject shadow = playerRef.transform.Find("PlayerCar/veh_shadow").gameObject;
    ...
    void InitializeButtonsBarStatus()
    {
        ...
        purchaseCar.transform.Find("PurchaseButton").gameObject.SetActive(false);
        purchaseCar.transform.Find("PurchaseButton").gameObject.SetActive(true);
...
//                OnTheRunOmniataManager.Instance.TrackVirtualPurchase(OmniataIds.Product_ParkingLot[currentSetIndex]/*OmniataIds.Product_ParkingLot + "_" + garageTitles[currentSetIndex]*/, PriceData.CurrencyType.SecondCurrency, parkingLotCost.ToString(), OmniataIds.Product_Type_Standard);
    ...
    IEnumerator wait(UIButton button)
    {
        ...
        else
        {
            if (OnTheRunFuelManager.Instance.Fuel <= 0 && !OnTheRunFuelManager.Instance.IsFuelFreezeActive())
            {
                if (PlayerPersistentData.Instance.FirstTimeFuelFinished && OnTheRunDataLoader.Instance.GetFirstFuelGiftEnabled())
                {
                    ...
                    uiPlayButton.gameObject.SetActive(false);
                   // GameObject.Find("GaragePage").SetActive(false);
                }
...
//		Debug.Log("canBeBought ============= "+ PlayerPersistentData.Instance.GetPlayerData(currentCarId).canBeBought);
        ...
        if (canAfford)
        {
            Transform pBut = purchaseCarLocked.transform.Find("PurchaseButton");

            if( pBut!= null )
                pBut.gameObject.SetActive(false);

			Transform p = button.gameObject.transform.parent.Find("tfBuyLabel");
			if( p != null )
...
*/
    ...
    void Signal_OnStatButtonPressed(UIButton button)
    {
        ...
        if (canAfford)
        {
			...
			Transform pStatPoint = infoBars[index].Find("Bar/barElement"+ iNextIndex);
...
/*            if (OnTheRunOmniataManager.Instance != null)
  ...
  */
    ...
    public void FillCarInfo(bool carChanged = false)
    {
        ...
        carStatsGroup.Find("tfCarName").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString(gameData.carName.ToString());//gameData.carName.ToString();
        ...
        purchaseCar.transform.Find("PurchaseButton/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(playerData.cost);
        purchaseCar.transform.Find("PurchaseButton/coin").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.FirstCurrency);
        purchaseCar.transform.Find("PurchaseButton/diamond").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.SecondCurrency);
...
*/
        ...
        infoBars[0].Find("tfDescription").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("acceleration");
        ...
        infoBars[1].Find("tfDescription").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("max_speed");
        ...
        infoBars[2].Find("tfDescription").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("resistance");

        nextUpgradeCost = OnTheRunEconomy.Instance.GetCarUpgradeCost(playerData, "turboSpeed");
        infoBars[3].Find("tfDescription").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("turbo_speed");
...
//#if !UNITY_WEBPLAYER
        ...
        else */if (categorySelectedCar[currentSetIndex] == 2)
        {
            if (carLockedByLevel)
            {
                ...
                purchaseCarLocked.transform.Find("PurchaseButton/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(playerData.cost);
                purchaseCarLocked.transform.Find("PurchaseButton/coin").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.FirstCurrency);
                purchaseCarLocked.transform.Find("PurchaseButton/diamond").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.SecondCurrency);
				purchaseCarLocked.transform.Find("Star/tfLevelValue").GetComponent<UITextField>().text = playerData.unlockAtLevel.ToString();

				purchaseCarLocked.transform.Find("PurchaseButtonCoins/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(playerData.alternativeCost);
            ...
            else if(carCanBeBought)
            {
                ...
                purchaseCarUnlocked.transform.Find("PurchaseButton/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(playerData.alternativeCost);
                purchaseCarUnlocked.transform.Find("PurchaseButton/coin").gameObject.SetActive(playerData.alternativeCurrency == PriceData.CurrencyType.FirstCurrency);
                purchaseCarUnlocked.transform.Find("PurchaseButton/diamond").gameObject.SetActive(playerData.alternativeCurrency == PriceData.CurrencyType.SecondCurrency);

                purchaseCarUnlocked.transform.Find("PurchaseButton2/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(playerData.cost);
                purchaseCarUnlocked.transform.Find("PurchaseButton2/coin").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.FirstCurrency);
                purchaseCarUnlocked.transform.Find("PurchaseButton2/diamond").gameObject.SetActive(playerData.currency == PriceData.CurrencyType.SecondCurrency);
...
//#endif
        ...
        if (PlayerPersistentData.Instance.IsParkingLotLocked(currentSetIndex))
        {
            ...
            */
            ...
            else
            {
                ...
                if (IsTierBlockedByXP(currentSetIndex) && !CheckTierLockedByXP(currentSetIndex))
                {
                    parkingLotLockerXP.transform.Find("tfXPTitle").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("unlock_tier_prefix");
                    parkingLotLockerXP.transform.Find("tfXP").GetComponent<UITextField>().text = playerLevelToUnlockTiers[currentSetIndex].ToString(); //OnTheRunDataLoader.Instance.GetLocaleString("level") + " " +
                    parkingLotLockerXP.transform.Find("tfXPText").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("unlock_tier_postfix");
                    parkingLotLockerXP.transform.Find("tfBuyFor").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("or_buy_for");

                    float buyForCost = tiersBuyFor[currentSetIndex];
                    if (buyForCost > 0.0f)
                    {
                        parkingLotBuyForButton.SetActive(true);
                        parkingLotBuyForButton.transform.Find("coin").gameObject.SetActive(false);
                        parkingLotBuyForButton.transform.Find("diamond").gameObject.SetActive(true);
                        parkingLotBuyForButton.transform.Find("tfTextfield").GetComponent<UITextField>().text = buyForCost.ToString();
                    }
            ...
            if (currentSetIndex > 0)
            {
                ...
                if (parkingLotLocker.activeInHierarchy)
                {
                    parkingLotLockedButton.SetActive(true);
                    PriceData.CurrencyType parkingLotCurrency = OnTheRunDataLoader.Instance.GetTiersCurrencies()[currentSetIndex];
                    parkingLotLockedButton.transform.Find("coin").gameObject.SetActive(parkingLotCurrency == PriceData.CurrencyType.FirstCurrency);
                    parkingLotLockedButton.transform.Find("diamond").gameObject.SetActive(parkingLotCurrency == PriceData.CurrencyType.SecondCurrency);
        ...
        for (int i = 0; i < 4; ++i)
        {
            currTr = transform.Find("AnchorCenter/Locations/locations_container").GetChild(i);
            ...
            Transform lockGroup = currTr.Find("lockGroup");
            if (hideLock)
                lockGroup.gameObject.SetActive(false);
            else
                lockGroup.Find("levelToUnlock").GetComponent<UITextField>().text = playerLevelToUnlockTiers[i].ToString();
        }

        currIcon = transform.Find("AnchorCenter/Locations/locations_container").GetChild(currentSetIndex).gameObject;
    ...
    public void FillInfoBar(Transform parent, int value, int minValue, int maxValue, bool carOwned, bool carLockedByDaily, bool forceEmpty = false, float upgradeCost = 0)
    {
        ...
        GameObject infoBar = parent.Find("Bar").gameObject;
        GameObject upgradeButton = parent.Find("GreenButtonSquare").gameObject;
        GameObject maxedTextfield = parent.Find("tfMaxed").gameObject;
        ...
        {
            Transform currElement = infoBar.transform.Find("barElement" + (i + 1));
            currElement.Find("empty").gameObject.SetActive(false);
            currElement.Find("full").gameObject.SetActive(true);
        }
        ...
        for (int i = minValue; i < maxValue; i++)
        {
			...
			Transform pEmpty = currElement.Find("empty");

			pEmpty.gameObject.SetActive(isEmpty || forceEmpty);
            currElement.Find("full").gameObject.SetActive(!isEmpty && !forceEmpty);
        }
        ...
        for (int i = maxValue; i < infoBar.transform.childCount; i++)
        {
            Transform currElement = infoBar.transform.Find("barElement" + (i + 1));
            currElement.Find("empty").gameObject.SetActive(false);
            currElement.Find("full").gameObject.SetActive(false);
        }
        ...
        {
            upgradeButton.transform.Find("tfPrice").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber((int)upgradeCost);
            upgradeButton.transform.Find("tfPrice").GetComponent<UITextField>().ApplyParameters();
...
//                        OnTheRunOmniataManager.Instance.TrackVirtualPurchase(OmniataIds.Product_ParkingLot[i]/*OmniataIds.Product_ParkingLot + "_" + garageTitles[currentSetIndex]*/, PriceData.CurrencyType.SecondCurrency, "0", OmniataIds.Product_Type_Unlock);
                    ...
                    Transform container = transform.Find("AnchorCenter/Locations/locations_container");
