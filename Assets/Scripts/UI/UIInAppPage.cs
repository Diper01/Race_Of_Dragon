...
/*#if UNITY_WEBPLAYER
        ...
        transform.Find("BottomRightAnchor/PlayButton/TextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("btPlay");
        playArrowEnable = PlayButton.transform.Find("Arrow_enable").gameObject;
        playArrowEnable.SetActive(true);
        playArrowDisable = PlayButton.transform.Find("Arrow_disable").gameObject;
...
//#endif
    ...
    void AddShopElements()
    {
        ...
        for (int i = 0; i < pageElements.Length; ++i)
        {
            ...
            currentElement.transform.Find("Background_active").gameObject.SetActive(false);
#if UNITY_WEBPLAYER
    ...
    void DisableBoosters(bool active)
    {
        ...
        {
            shopItems[i].transform.Find("btMark").GetComponent<BoxCollider2D>().enabled = !active;
//            shopItems[i].transform.FindChild("BuyBoosterButton").GetComponent<BoxCollider2D>().enabled = !active;
            shopItems[i].transform.Find("Item/ItemButton").gameObject.SetActive(!active);
        }
    }
    ...
    bool FillData(GameObject shopElement, int elementIndex)
    {
        ...
        switch (elementIndex)
        {
            case 0:
                ...
                shopItemComp.transform.Find("bg_selection_prize").gameObject.SetActive(false);
                if (TruckBehaviour.randomTruckFromInAppPage == TruckBehaviour.TrasformType.None || missionRequireSpecialVehicle)
                {
                    ...
                    {
                        shopElement.transform.Find("Background_normal").gameObject.SetActive(false);
                        shopItemComp.transform.Find("bg_selection_prize").gameObject.SetActive(true);
                ...
                SpriteRenderer spriteRenderer = shopItemComp.transform.Find("Item/Icons/Icon5").GetComponent<SpriteRenderer>();
...
//        shopElement.transform.FindChild("BuyBoosterButton/diamond").gameObject.SetActive(currBooster.priceData.Currency == PriceData.CurrencyType.SecondCurrency);
        ...
        shopElement.transform.Find("Background_normal").gameObject.SetActive(!boosterEquipped);
        shopElement.transform.Find("Background_active").gameObject.SetActive(boosterEquipped);
        if (boosterEquipped && elementIndex==0)
            shopItemComp.transform.Find("bg_selection_prize").gameObject.SetActive(false);

        if (currBooster.type == OnTheRunBooster.BoosterType.SurprisePack || currBooster.type == OnTheRunBooster.BoosterType.MultiplePack)
        {
            shopElement.transform.Find("Background_normal").gameObject.SetActive(true);
            shopElement.transform.Find("Background_active").gameObject.SetActive(false);
        }

        //Icon----------------------------
        Transform node = shopElement.transform.Find("Item/Icons");
        ...
        switch (currBooster.type)
        {
            case OnTheRunBooster.BoosterType.MultiplePack:
                node.Find("Icon1").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.SurprisePack:
                node.Find("Icon2").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.DoubleCoins:
                node.Find("Icon3").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.MoreCheckpointTime:
                node.Find("Icon4").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.SpecialCar:
                node.Find("Icon5").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.Turbo:
                node.Find("Icon6").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.DoubleExp:
                node.Find("Icon7").gameObject.SetActive(true);
                break;
            case OnTheRunBooster.BoosterType.Shield:
                node.Find("Icon8").gameObject.SetActive(true);
                break;
        }
	...
	void Update()
	{
		...
		//if(Advertisement.IsReady() && freeBooter == false)
		//{
		//	freeBoosterElement.SetActive(true);
		//}
		//else
		//{
		//	freeBoosterElement.SetActive(false);
		//}
	}
	//-------------------------------------------//
    ...
    {
		print ("----------------Free Bosster Video Press----------------------");
...
*/		
    ...
    IEnumerator Signal_OnPlayPressed(UIButton button)
    {
        ...
        /*while (true/*UIEnterExitAnimations.activeAnimationsCounter > 0)
        {
            yield return null;
        }*/
    ...
    void UpdateAllButtons()
    {
        ...
        GameObject buttonsRoot = gameObject.transform.Find("CenterAnchor/Buttons").gameObject;
