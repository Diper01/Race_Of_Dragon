...
public class UIIngameMissionsAnimation : MonoBehaviour
{
    ...
    void Initialize()
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            currentRow.Find("tfDescription").GetComponent<UITextField>().text = " ";
            currentRow.Find("tfValue").gameObject.SetActive(false);
            currentRow.Find("icon_mission_completed").gameObject.SetActive(false);
        ...
        for (int i = 0; i < 1; ++i)
        {
            ...
            if (currMission != null)
            {
                ...
                currentRow.Find("tfDescription").GetComponent<UITextField>().text = currMission.description;
                currentRow.Find("tfValue").GetComponent<UITextField>().text = uiRoot.FormatTextNumber((int)(Mathf.Floor(currMission.Counter))) + "/" + uiRoot.FormatTextNumber(currMission.checkCounter);
                currentRow.Find("tfValue").gameObject.SetActive(!missionPassed);
                currentRow.Find("icon_mission_completed").gameObject.SetActive(missionPassed);
                currentRow.Find("NewsBadge").gameObject.SetActive(OnTheRunDailyBonusManager.Instance.NewsBadgdesActive);
                if (currentRow.Find("NewsBadge").gameObject.activeInHierarchy)
                    currentRow.Find("NewsBadge").GetComponentInChildren<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("mission_new");
                ++currentRowIndex;
                ++rowsToShow;
            }
        }
