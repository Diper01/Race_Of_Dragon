...
public class UIRankScroller : MonoBehaviour
{
    ...
    public static List<(int level, Sprite sprite)> rewardSprites = new List<(int level, Sprite sprite)>();
    ...
    void InitRankScroller(bool isPopup)
    {
        ...
        float elementBaseWidth = tmpElement.transform.Find("grid/horizontal_bar").GetComponent<SpriteRenderer>().bounds.size.x * 0.985f;
        ...
        transform.Find("Scroller/Mask").GetComponent<SpriteRenderer>().sortingOrder = 0; // baseSortingOrder;
    ...
    {
        transform.Find("Scroller/RankingScroller/offsetNode").transform.localScale = Vector3.one;
    }
    ...
    void AddPlayerElement()
    {
        ...
        playerBar = newElement.transform.Find("Bar");
    ...
    void Update()
    {
        if (PlayerPersistentData.Instance.Level == 35)
        {
    ...
    void AddBonusConsumableElement(float xPos, OnTheRunConsumableBonusManager.ConsumableBonus cons)
    {
        ...
        rewardSprites.Add((cons.level, newElement.transform.Find("icon").GetComponent<SpriteRenderer>().sprite));
    }
    #endregion
