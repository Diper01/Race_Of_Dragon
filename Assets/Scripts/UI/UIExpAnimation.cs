...
public class UIExpAnimation: MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (m_pFuelsAnim == null)
            m_pFuelsAnim = m_pUIRoot.transform.Find("CurrencyBar/FuelItem/FuelNormal/fuel").gameObject.GetComponent<Animation>();

        if (m_pCoinsAnim == null)
        {
            Transform coinAnimTr = m_pUIRoot.transform.Find("CurrencyBar/CoinsItem/GreenButtonSquare/Normal/coin");
            if(coinAnimTr == null)
                coinAnimTr = m_pUIRoot.transform.Find("CurrencyBar/CoinsItem/coin");
            m_pCoinsAnim = coinAnimTr.gameObject.GetComponent<Animation>();
        }

        if (m_pMetersAnim == null)
            m_pMetersAnim = m_pUIRoot.transform.Find("TopLeftObjects/LevelBar/star_fb_user").gameObject.GetComponent<Animation>();
        ...
        if (m_pCoinsText == null)
            m_pCoinsText = m_pUIRoot.transform.Find("CurrencyBar/CoinsItem/ValueTextField").gameObject.GetComponent<UITextField>();
...
//#else
			m_pMetersText = GameObject.Find("RewardPage").transform.Find("CenterScreenAnchor/AnimAnchor/Meters/ValueTextField").gameObject.GetComponent<UITextField>();
//#endif

        if (m_pDiamondsText == null)
            m_pDiamondsText = m_pUIRoot.transform.Find("CurrencyBar/DiamondsItem/ValueTextField").gameObject.GetComponent<UITextField>();
...
//#else
            m_pCoinsCollectedText = GameObject.Find("RewardPage").transform.Find("CenterScreenAnchor/AnimAnchor/Coins/ValueTextField").gameObject.GetComponent<UITextField>();
//#endif
    ...
    void UpdateFloatingFuels()
    {
        ...
        for (int i = 0; i < POOL_SIZE; ++i)
        {
            ...
            if (Vector3.Distance(kFuel.transform.position, m_pFuelsAnim.transform.position) < 0.15f)
            {
                ...
                Manager<UIRoot>.Get().transform.Find("CurrencyBar").GetComponent<UICurrencyBarItem>().SetFuelValue();
                // play the bounce animation
                m_pFuelsAnim.Rewind();
                m_pFuelsAnim.Play();
                interfaceSounds.CounterCoin();
    ...
    {
        for (int i = 0; i < POOL_SIZE; ++i)
            if (m_aDiamonds[i]!=null&&m_aDiamonds[i].activeSelf == false)
