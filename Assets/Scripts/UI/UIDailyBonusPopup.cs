...
public class UIDailyBonusPopup : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (daysObjects == null)
        {
            ...
            for (int i = 0; i < OnTheRunDailyBonusManager.MaxDailyBonusDays - 1; ++i)
            {
                ...
                curr.transform.parent = gameObject.transform.Find("content/Items");
                curr.transform.localPosition = new Vector3(xPositions[i], currYPos, 0.0f);

                if (i == OnTheRunDailyBonusManager.MaxDailyBonusDays - 2)
                {
                    //day 10
                    curr.transform.Find("dayten_bg").gameObject.SetActive(true);
                    curr.transform.Find("fireworks").gameObject.SetActive(true);
                } 
                else if (i == 3)
                {
                    //day 5
                    curr.transform.Find("dayfive_bg").gameObject.SetActive(true);
                    curr.transform.Find("normal_bg").gameObject.SetActive(false);
                }
    ...
    void Start()
    {
        OnTheRunUITransitionManager.Instance.InitializeBackground(transform.Find("Backgrounds/bg_black_gradient"));
    }
    #endregion
    ...
    void OnShow(UIPopup popup)
    {
        ...
        collectButton.transform.Find("tfTextfield").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("collect");
        collectButton.transform.Find("tfTextfield").GetComponent<UITextField>().ApplyParameters();
        ...
        for (int i = 0; i < daysObjects.Count; ++i)
        {
            ...
            if (dayToday && data.bonusType == OnTheRunDailyBonusManager.DailyBonus.Mistery) //misteryGiftIndexes.Contains(currentDayIdx))
            {
                ...
                alphaFade = -1;
                alphaValue = 1.0f;
                currDay.transform.Find("fireworks").gameObject.SetActive(true);
            }
    ...
    void FadeOutCompleted()
    {
        ...
        itemAnimated.transform.parent.transform.Find("tfPrize").GetComponent<UITextField>().text = misteryReward.quantity.ToString();
        alphaFade = 1;
        alphaValue = 0.0f;
    }
    ...
    void FadeInCompleted()
    {
        ...
        alphaFade = 0;
        collectButton.SetActive(true);
        itemAnimated.transform.parent.transform.Find("fireworks").gameObject.SetActive(false);
        collectButton.transform.Find("tfTextfield").GetComponent<UITextField>().ApplyParameters();
    }
...
*/
    ...
    {
        Transform collectButton = transform.Find("content/CollectButton");
