...
public class UIRewardPage : MonoBehaviour
{
    ...
    void Awake()
    {
        bottomBar = Manager<UIRoot>.Get().transform.Find("RewarBottomBar").gameObject;
    ...
    void OnEnable()
    {
        ...
        Transform centralRoot = transform.Find("CenterScreenAnchor");
        transform.Find("CenterScreenAnchorWeb").gameObject.SetActive(false);
...
//#endif
        ...
        centralRoot.Find("AnimAnchor/Coins/LabelTextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("coins");
//#if UNITY_WEBPLAYER
...
//#else
        UITextField metersTextfield = centralRoot.Find("AnimAnchor/Meters/LabelTextField").GetComponent<UITextField>();
        ...
        centralRoot.Find("AnimAnchor/Meters/BestLabelTextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("best_meters"); //"best"
//#endif
		...
		if( m_pBestMetersText == null )
            m_pBestMetersText = centralRoot.Find("AnimAnchor/Meters/BestValueTextField").gameObject.GetComponent<UITextField>();
		
        if( m_pCurrentMetersText == null )
            m_pCurrentMetersText = centralRoot.Find("AnimAnchor/Meters/ValueTextField").gameObject.GetComponent<UITextField>();

        if (m_pBestMetersLabel == null)
            m_pBestMetersLabel = centralRoot.Find("AnimAnchor/Meters/BestLabelTextField").GetComponent<UITextField>();

        if (newRecord == null)
        {
            newRecord = centralRoot.Find("AnimAnchor/Meters/NewRecord").gameObject;
        ...
        if (!alreadyEntered)
        {
            ...
            centralRoot.Find("AnimAnchor/Coins/ValueTextField").GetComponent<UITextField>().text = "0";
        ...
        {
            centralRoot.Find("AnimAnchor/Meters/ValueTextField").GetComponent<UITextField>().text = sessionMeters.ToString();
            centralRoot.Find("AnimAnchor/Coins/ValueTextField").GetComponent<UITextField>().text = sessionCoins.ToString();
        }
    ...
    IEnumerator ExpRewardDelayed(int expForMeters, int sessionCoins)
    {
        ...
        if (expForMeters > 0)
            Manager<UIRoot>.Get().GetComponent<UIExpAnimation>().StartExpAnimation((int)(expForMeters), (int)(expForMeters), centralRoot.Find("AnimAnchor/Meters/ValueTextField").gameObject, 4.0f, 0f, true, true);
        else
            Manager<UIManager>.Get().ActivePage.SendMessage("StartNewRecordAnimation");

        if (sessionCoins > 0)
            m_pUIRoot.GetComponent<UIExpAnimation>().StartCoinsAnimation(sessionCoins, centralRoot.Find("AnimAnchor/Coins/ValueTextField").gameObject, 0.1f, 0.4f);
    }
    ...
    void OnPlatformsPopupClosed()
    {
        if (!canAnimationBegin)
        {
            ...
            if (xpGained > 0)
                m_pUIRoot.GetComponent<UIExpAnimation>().StartExpAnimation(xpGained, sessionMeters, centralRoot.Find("AnimAnchor/Meters/ValueTextField").gameObject, 3.0f, 0.4f);  //sessionMeters
            if (sessionCoins > 0)
                m_pUIRoot.GetComponent<UIExpAnimation>().StartCoinsAnimation(sessionCoins, centralRoot.Find("AnimAnchor/Coins/ValueTextField").gameObject, 0.1f, 0.4f);
...
*/
	...
	//-------------------------------------------------------//
     ...
     IEnumerator UpdateBestMeterText(float delayTime)
     {
         ...
         if (showFireworks)
            OnTheRunFireworks.Instance.StartFireworksEffect(5, Manager<UIManager>.Get().ActivePage.gameObject.transform.Find("fireworks"));
     }
