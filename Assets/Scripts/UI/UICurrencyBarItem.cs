...
public class UICurrencyBarItem : MonoBehaviour
{
    ...
    { //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////CHEAT cheat Cheat
      // cheats
      //PlayerPersistentData.Instance.Coins = 100000;
      //PlayerPersistentData.Instance.Diamonds = 10000;
      // вечная заморозка энергии 
      //OnTheRunFuelManager.Instance.StartFuelFreeze();

        // Manager<UIRoot>.Get().FormatTextNumber((int)PlayerPersistentData.Instance.Coins) = 1000;
    ...
    void OnShowBar()
    {
        ...
        fuelBar = transform.Find("FuelItem/FuelNormal/FuelBar").gameObject;
        tfFuelTime = transform.Find("FuelItem/FuelNormal/tfFuelTime").GetComponent<UITextField>();
        tfFuelValue = transform.Find("FuelItem/FuelNormal/tfFuelValue").GetComponent<UITextField>();

        transform.Find("FuelItem/FuelFreeze/tfFuelFreezeText").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("freeze");
    ...
    {
        transform.Find("CoinsItem/ValueTextField").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber((int)PlayerPersistentData.Instance.Coins);
    }
    ...
    void CreateFuelBar()
    {
        ...
        if (fuelBar.transform.childCount != fuelBarElementsNum)
        {
            ...
            {
                DestroyImmediate(fuelBar.transform.Find("barElement" + (i + 1)).gameObject);
            }
    ...
    public void SetFuelValue()
    {
        ...
        {
            bool isEmpty = true;
            Transform currElement = fuelBar.transform.Find("barElement" + (i + 1));
            if ((i + 1) <= OnTheRunFuelManager.Instance.Fuel)
                isEmpty = false;

            currElement.Find("empty").gameObject.SetActive(isEmpty);
            currElement.Find("full").gameObject.SetActive(!isEmpty);
        }
    ...
    {
        if (popupType == UICurrencyPopup.ShopPopupTypes.Diamonds)
            yield break;
