...
public class UIStartPage : MonoBehaviour
{
    ...
    void OnEnable()
    {
        ...
        if (newsBadge == null)
        {
            newsBadge = NewsItem.transform.Find("Notification").gameObject;
            newsIconRenderer = NewsItem.transform.Find("option_med").GetComponent<SpriteRenderer>();
            newsIconText = NewsItem.transform.Find("tfOptionsLabel").GetComponent<UITextField>();
            newsIconStroke = NewsItem.transform.Find("tfOptionsLabel/stroke").GetComponent<UITextField>();
        }
...
//#endif
    ...
    void Update()
    {
        ...
        if (playerRef!=null)
            playerRef.transform.Find("PlayerCar").gameObject.SetActive(false);
...
//		FreeCoinsItem.GetComponent<UIButton>().State = (OnTheRunCoinsService.Instance.IsVideoAdAvailable()) ? UIButton.StateType.Normal : UIButton.StateType.Disabled;
    ...
    void RefreshSpinWheelButton( )
    {
        ...
        Manager<UIRoot>.Get().SetupWheelButtonBounce(transform.Find("AnchorBottomLeft/btSpinWheel").gameObject);
    }
...
#elif UNITY_ANDROID && UNITY_KINDLE && !UNITY_EDITOR
        string bundleId = "com.codostudio.pogamongo";
		Application.OpenURL("https://www.amazon.com/s/ref=bl_sr_mobile-apps/168-2235421-3771920?_encoding=UTF8&field-brandtextbin=Codo%20Studio&node=2350149011");
#elif UNITY_WP8 && !UNITY_EDITOR
		SBS.Miniclip.WP8Bindings.OnMoreGames("https://www.facebook.com/bidostudio1211/");
#elif UNITY_WEBPLAYER
		Application.ExternalEval("window.open('https://www.facebook.com/bidostudio1211/','_blank')");
#endif
    }
    ...
    {
        interfaceSounds.SendMessage("PlayGeneralInterfaceSound", OnTheRunInterfaceSounds.InterfaceSoundsType.Click);
		Debug.Log("FREE COINS: Signal_OnFreeCoinsRelease ---- Free Coin Press ----");
    ...
    public void UpdateSpinWheelButton()
    {
        ...
        {
            transform.Find("AnchorBottomLeft/btSpinWheel/Remaining").gameObject.SetActive(true);
            transform.Find("AnchorBottomLeft/btSpinWheel/Remaining/tfRemaining").GetComponent<UITextField>().text = remainingSpinds.ToString();
        }
        else
            transform.Find("AnchorBottomLeft/btSpinWheel/Remaining").gameObject.SetActive(false);
    }
    ...
    public void UpdateNewsfeedButton()
    {
        ...
        if (totalNews > 0)
        {
            ...
            NewsItem.transform.Find("Notification/tfNotification").gameObject.SetActive(false);
            if (unreadNews > 0)
            {
                newsBadge.gameObject.SetActive(true);
                NewsItem.transform.Find("Notification/tfNotification").gameObject.SetActive(true);
                NewsItem.transform.Find("Notification/tfNotification").GetComponent<UITextField>().text = unreadNews.ToString();
            }
            else
...
//        shadow.transform.localPosition = new Vector3(shadow.transform.localPosition.x, shadow.GetComponent<PlayerShadow>().shadowVerticalOffset, shadow.transform.localPosition.z);
    ...
    void InitTierScroller()
    {
        ...
        for (int i = 0; i < tiersCount; i++)
        {
            ...
            newTier.transform.Find("PlayButton").GetComponent<UIButton>().onReleaseEvent.AddTarget(gameObject, "PlayButtonReleased");
            newTier.transform.Find("ClickArea").GetComponent<UIButton>().onReleaseEvent.AddTarget(gameObject, "ClickAreaReleased");
			newTier.transform.Find("LockedArea/btBuyTier/btBuyTierbtn").GetComponent<UIButton>().onReleaseEvent.AddTarget(gameObject, "BuyTierReleased");
        }
