...
public class UITrucksPage : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        tfGarageGenericDescr = transform.Find("AnchorBottomRight/tfGarageTitle").GetComponent<UITextField>();
        ...
        panel = transform.Find("LeftCenterAnchor/InfoPanel/BuyPopup").gameObject;

        //uiToggle = gameObject.transform.FindChild("Toggle").GetComponent<UIToggleButton>();

        vehiclesScroller = transform.Find("AnchorCenter/VehiclesScroller").GetComponent<UIScroller>();
...
//                OnTheRunOmniataManager.Instance.TrackVirtualPurchase(OmniataIds.Product_Special + "_" + carType.ToString().ToLowerInvariant(), PriceData.CurrencyType.SecondCurrency, carCost.ToString(), OmniataIds.Product_Type_Standard);
    ...
    public void OnMoreDurationPressed(TruckBehaviour.TrasformType carType)
    {
        ...
        if (canAfford)
        {
			...
			Transform pStatPoint = InfoItem.Find("Bar/barElement"+ iNextIndex);
...
//			lockItem.transform.FindChild("BuyPanel").gameObject.SetActive( lockAnimPlaying == false );
        ...
        if (lockItem.activeInHierarchy)
        {
			...
			lockItem.transform.Find("BuyPanelLocked/Star/tfLevelValue").GetComponent<UITextField>().text = unlockAtXPLevel.ToString();
#if !UNITY_WEBPLAYER
			lockItem.transform.Find("BuyPanelLocked/PurchaseButton/tfTextfield").GetComponent<UITextField>().text = data.cost.ToString();
#else
...
#endif
			lockItem.transform.Find("BuyPanelLocked/PurchaseButton/coin").gameObject.SetActive(data.currency == PriceData.CurrencyType.FirstCurrency);
			lockItem.transform.Find("BuyPanelLocked/PurchaseButton/diamond").gameObject.SetActive(data.currency == PriceData.CurrencyType.SecondCurrency);

            lockItem.transform.Find("PurchaseButtonCoins/tfTextfield").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(data.alternativeCost);
        } 
        else if(buyButtons.activeInHierarchy)
        {
            buyAlternative.transform.Find("PriceTextField").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(data.alternativeCost);
            buyAlternative.transform.Find("Currency/coin").gameObject.SetActive(data.alternativeCurrency == PriceData.CurrencyType.FirstCurrency);
            buyAlternative.transform.Find("Currency/diamond").gameObject.SetActive(data.alternativeCurrency == PriceData.CurrencyType.SecondCurrency);

            buyNormal.transform.Find("PriceTextField").GetComponent<UITextField>().text = Manager<UIRoot>.Get().FormatTextNumber(data.cost);
            buyNormal.transform.Find("Currency/coin").gameObject.SetActive(data.currency == PriceData.CurrencyType.FirstCurrency);
            buyNormal.transform.Find("Currency/diamond").gameObject.SetActive(data.currency == PriceData.CurrencyType.SecondCurrency);
        }
    }
}