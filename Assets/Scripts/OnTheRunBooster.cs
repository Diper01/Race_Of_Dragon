...
public class OnTheRunBooster
{
    ...
    public OnTheRunBooster()
    {
        ...
        buyEffect = uiRoot.transform.Find("FxUIBuy").GetComponent<ParticleSystem>();
        buyEffect.Stop();
    ...
    public bool IsBoosterEquipped(BoosterType type)
    {
        ...
        //.Assert(false, "IsBoosterEquipped Exception: no booster " + type + "found.");
        return false;
    }
...
*/
                ...
                else if (!canBuy)
                {
                    ...
                    Manager<UIManager>.Get().FrontPopup.transform.Find("content/popupTextField").GetComponent<UITextField>().text = OnTheRunDataLoader.Instance.GetLocaleString("you_already_have") + " " + currData.name.ToUpper() + " " + OnTheRunDataLoader.Instance.GetLocaleString("boost");
                }
                else
                {
    ...
    public int EquippingBooster(BoosterType type, bool equipped)
    {
        ...
        {
            OnTheRunFireworks.Instance.StartFireworksEffect(25, Manager<UIManager>.Get().ActivePage.gameObject.transform.Find("fireworks"));
            for (int i = 0; i < boosters.Count; ++i)
            {
                Booster currData = boosters[i];
 ...
 //       if (!found)
            //.Assert(false, "OnEquipBooster Exception: no booster " + type + "found.");
