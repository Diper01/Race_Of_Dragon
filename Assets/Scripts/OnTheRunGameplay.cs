...
public class OnTheRunGameplay : MonoBehaviour
{
	...
	void Awake()
	{
		...
		//.Assert(player != null, "OnTheRunGameplay.Awake() - Could not find GameObject with Tag \"Player\".");
		Instance = this;
	...
	IEnumerator LoadEnvironment(bool preload)
	{
		...
		OnTheRunObjectsPool.Instance.AddNewItems(EnvironmentSceneGo.transform.Find("AdditionalItems").GetComponent<AdditionalPoolElements>().additionalPoolList);
